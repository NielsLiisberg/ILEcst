...
# to try this out:
# python extract.py --host MY_IBM_I  --liblist QGPL,QUSRSYS --pgm hello --source QGPL/QRPGLESRC  --output cst.json 
...
# Just for now, stack symbol_tables as they are
# ------------------------------------------------------------------------
def push_source_table (source_table, ln):
...
def run_script(host, out, cmd ):
	...
	for ln in proc.stdout:
		...
		if 	 state =='code':
			push_source_table (source_table , ln)
		elif state == 'symbols':
...
parser.add_argument('--pgm', help='Program (member) to compile and extract')
...
source = args.source
output = args.output if pgm == None else pgm + '.extract.json'

#print args
#print sys


run_script ( host, output, 
